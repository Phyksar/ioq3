...
static int gametype_remap[] = {GT_FFA, GT_TEAM, GT_TOURNAMENT, GT_CTF, GT_HEADHUNT};
static int gametype_remap2[] = {0, 2, 0, 1, 3, 3, 3, 3, 4};
...
static qboolean BotAlreadySelected( const char *checkName ) {
	...
	for( n = 1; n < PLAYER_SLOTS; n++ ) {
		...
		if ((GametypeIsTeam(s_serveroptions.gametype)) &&
...
static void ServerOptions_Start( void ) {
	...
	// set player's team
	if (dedicated == 0 && GametypeIsTeam(s_serveroptions.gametype)) {
...
static void ServerOptions_InitPlayerItems( void ) {
	...
	if (s_serveroptions.multiplayer && !GametypeIsTeam(s_serveroptions.gametype)) {
